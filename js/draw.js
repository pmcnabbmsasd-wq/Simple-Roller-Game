...
Game.startLevel = function (levelNumber) {
  ...
  Player.reset();
  if (levelNumber === 1) { Game.spawnGoombas(); }
  Game.mode = "playing";
  Game.showLives();
  Game.showAmmo();
  Game.showLevel();
...
Game.spawnGoombas = function () {
  Game.goombas = [
    { x: 270, y: 280, vx: CONFIG.GOOMBA_SPEED, size: CONFIG.GOOMBA_SIZE, alive: true },
    { x: 475, y: 240, vx: -CONFIG.GOOMBA_SPEED, size: CONFIG.GOOMBA_SIZE, alive: true },
    { x: 710, y: 200, vx: CONFIG.GOOMBA_SPEED, size: CONFIG.GOOMBA_SIZE, alive: true },
    { x: 960, y: 260, vx: -CONFIG.GOOMBA_SPEED, size: CONFIG.GOOMBA_SIZE, alive: true }
  ];
};
...
Game.updateGoombas = function () {
  for (var i = 0; i < Game.goombas.length; i++) {
    ...
    goomba.x = goomba.x + goomba.vx;
    if (goomba.x < 80 || goomba.x > Level.pixelWidth() - 80) {
      goomba.vx = -goomba.vx;
      goomba.x = Math.max(80, Math.min(goomba.x, Level.pixelWidth() - 80));
    }
  }
};

Game.resolveGoombaCollisions = function () {
  for (var i = 0; i < Game.goombas.length; i++) {
    var goomba = Game.goombas[i];
    if (!goomba.alive) { continue; }

    if (Collide.overlaps(Player.x, Player.y, CONFIG.PLAYER_SIZE, CONFIG.PLAYER_SIZE,
      goomba.x, goomba.y, goomba.size, goomba.size)) {
      var playerBottom = Player.y + CONFIG.PLAYER_SIZE;
      var stomping = Player.vy > 0 && playerBottom - Player.vy <= goomba.y + 8;

      if (stomping) {
        goomba.alive = false;
        Player.vy = -8;
        Game.showMessage("Goomba stomp!");
        return;
      }
